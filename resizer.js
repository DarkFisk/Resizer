(function(){
	...
	var Resizer = {
		...
		init: function(){
			...
			Resizer.$container = $('<div id="responsive_resize_container"></div>').appendTo('body');
			$('<div class="resize_inner"><a class="resizer_bigger" href="#">+</a><a class="resizer_smaller" href="#">-</a><a class="resize_drop_selector" href="#"><span class="resizer_active_dimension">Mobile <span class="resizer_dimension">(320x480)</span></span></a><ul class="resize_dropdown"></ul><a class="reload" href="#">Reload</a><a class="reset" href="#">Reset</a></div>').appendTo(Resizer.$container);
			Resizer.$container_dropdown = $('ul.resize_dropdown', Resizer.$container);
			$.each(Resizer.sizes, function(index, value){
				$('<li data-dimension="'+ index +'" data-description="'+ value.description +'" data-height="'+ value.height +'" data-width="'+ value.width +'"><a href="#">'+ value.description +'</a></li>').appendTo(Resizer.$container_dropdown);
			});
			$('a.resize_drop_selector').click(function(e){
				Resizer.$container_dropdown.show();
				e.preventDefault();
			});
			
			$('a.resizer_smaller', Resizer.$container).click(function(e){
				Resizer.smaller();
				e.preventDefault();
			});
			$('a.resizer_bigger', Resizer.$container).click(function(e){
				Resizer.bigger();
				e.preventDefault();
			});
			
			$('a.reset', Resizer.$container).click(function(e){
				Resizer.reset();
				e.preventDefault();
			});
			
			$('a.reload', Resizer.$container).click(function(e){
				Resizer.reload();
				e.preventDefault();
			});
			
			$('li a', Resizer.$container_dropdown).click(function(e){
				$data = $(this).parent().data();
				Resizer.resize($data.width, $data.height);
				Resizer.$container_dropdown.hide();
				Resizer.current_dimension = $data.dimension;
				$('.resizer_active_dimension').html(  $data.description +' <span class="resizer_dimension">('+ $data.width +'x'+ $data.height +')</span>');
				e.preventDefault();
			});
		},
		...
		resize: function(width, height){
			$('#resizer_resize_wrapper').remove();
			$wrapper = $('<div id="resizer_resize_wrapper"></div>').appendTo(Resizer.$target);
			$('<iframe />', {
				name: 'resizer_frame',
				id:   'resizer_frame',
		...
		reload: function(){
			$('#resizer_frame').attr('src', window.location + '?' + Math.floor(Math.random()*11));
		},
		...
		bigger: function(){
			if(Resizer.current_dimension != 'desktop'){
				...
				$('.resizer_active_dimension').html(  next_dimension.description +' <span class="resizer_dimension">('+ dimensions.width +'x'+ dimensions.height +')</span>');
		...
		loadCss: function(){
			if($('head link#responsive_css').length == 0){						
				$('<link>',{
